...
def hook():
    ...
    try:
        ...
        # De-dupe same message_id per user
        ...
        # Current persona (always resolve with modulo)
        p = PERS[s["g"] % len(PERS)]
        ...
        # ----- Diagnostics (safe, self-contained) -----
        if low.startswith("/diag"):
            try:
                checks = []
                checks.append(f"BOT_TOKEN set: {bool(BOT_TOKEN)}")
                checks.append(f"OWNER_ID set: {bool(OWNER_ID)}")
                checks.append(f"WEBHOOK_URL set: {bool(WEBHOOK_URL)}")
                checks.append(f"FAL_KEY set: {bool(FAL_KEY)}")
                checks.append(f"REPLICATE_API_TOKEN set: {bool(REPLICATE)}")
                checks.append(f"HORDE_API_KEY set: {bool(HORDE)}")
                checks.append(f"PERS loaded: {bool(PERS)} (count={len(PERS) if 'PERS' in globals() else 0})")
                checks.append(f"Current index: {s['g']} valid: {0 <= s['g'] < len(PERS) if PERS else False}")
                checks.append(f"NSFW: {s.get('nsfw')}, Used today: {s.get('used')}/{FREE_PER_DAY}")
                checks.append(f"Likes: {', '.join(s.get('likes', [])) or '(none)'}")
                try:
                    r = requests.get(f"{API}/getMe", timeout=8)
                    checks.append(f"Telegram getMe: {r.status_code}")
                except Exception as e_ping:
                    checks.append(f"Telegram getMe: error {e_ping}")
                send_message(chat, "🔧 DIAG\n" + "\n".join(checks))
            except Exception as e_diag:
                send_message(chat, f"/diag error: {e_diag}")
            return "OK", 200
        ...
        # =================== Bare number/name switching ===================
        # If user sends just "13" or "Brittany", treat as quick pick.
        ...
        # One-time tease after some messages
