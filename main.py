...
FREE_PER_DAY = 2
PRIVATE_ONLY = True
FORBID = {"minor","underage","child","incest",
...
def token_cmd(m):
    ...
    if a[1].strip() in TOKEN_CODES:
        gu(m.from_user.id)["ok"]=True; save_state()
        bot.reply_to(m,"✅ Unlimited unlocked.")
    else:
...
def set_webhook():
    if not PUBLIC_URL:
        print("PUBLIC_URL missing; set it and redeploy.")
        return
    try:
