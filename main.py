...
WEBHOOK_SECRET = os.getenv("WEBHOOK_SECRET","pix3lhook")
RUN_MODE = os.getenv("RUN_MODE","webhook")  # "webhook" or "polling"
...
# >>> YOUR GIRLS (edit names/vibes here) <<<
PERS=[
    ("Nicole","playful"),
...
<p class=muted>Mode: <b>{{mode}}</b> · Free/day: <b>{{free}}</b></p>
<div class=card><p>Status: <b>{{status}}</b> · Horde key: <b>{{horde}}</b></p>
...
def home():
    ...
    return render_template_string(HOME,status=status,horde=hk,free=FREE_PER_DAY,girls=PERS,bot_username=name.strip("@"),mode=RUN_MODE)
@app.route("/_up")
def up(): return "ok",200
@app.route("/mode")
def mode(): return RUN_MODE,200
...
def set_webhook():
    if RUN_MODE!="webhook": return
    if not PUBLIC_URL:
        print("PUBLIC_URL missing; set it and redeploy."); return
    try:
        hook=f"{PUBLIC_URL}/telegram/{WEBHOOK_SECRET}"
        requests.post(f"https://api.telegram.org/bot{TG_TOKEN}/deleteWebhook",timeout=10)
        r=requests.post(f"https://api.telegram.org/bot{TG_TOKEN}/setWebhook",json={"url":hook},timeout=15)
        j=r.json()
        if not j.get("ok"):
            print("Webhook set failed:", j)
        else:
...
def run_polling():
    if RUN_MODE=="polling":
        try: requests.post(f"https://api.telegram.org/bot{TG_TOKEN}/deleteWebhook",timeout=10)
        except: pass
        bot.send_message(int(OWNER_ID),"✅ READY — polling")
        bot.infinity_polling()

if __name__=="__main__":
    load_state()
    threading.Thread(target=set_webhook,daemon=True).start()
    threading.Thread(target=run_polling,daemon=False).start() if RUN_MODE=="polling" else None
